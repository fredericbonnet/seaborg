...
describe('docRefTextType', () => {
  const render = (xml: string) => {
    const {
      children: [root],
    } = parseXml(xml);
    return docRefTextType(root as Element);
  };
  ...
  specify('member with markup', async () => {
    const xml = `<ref refid="member_67890" kindref="member"><bold>some text</bold></ref>`;
    const md = '[**some text**](compound_12345.md#member_67890)';
    expect(render(xml)).to.equal(md);
  });
});
